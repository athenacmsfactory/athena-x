...
export function detectCurrentTheme(siteDir) {
    // Strategy 1: Check index.css for @import "./css/xxx.css"
    ...
    // Strategy 3: Check athena-config.json
...
export function generateVariants(siteName, options = {}) {
    ...
    const targetThemes = options.styles
        ? options.styles.filter(s => allThemes.includes(s))
        : allThemes;

    // Filter out the current theme
    const themesToGenerate = targetThemes.filter(t => t !== currentTheme);
